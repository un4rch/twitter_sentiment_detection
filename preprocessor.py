...
class Preprocessor:
    ...
    def preprocessDataset(self, pMl_dataset, pTargetColumn, pAlgorithm, pExcludedColumns, pImputeOption, pRescaleOption, pNLcolumns, pNLtechnique, pTarinTest, pSwitch):
        ...
        #target_map = self.convertTargetToClassifyInt(pMl_dataset, pTargetColumn)
    ...
    def preprocessEvolved(self, pMl_dataset, pTargetColumn, pExcludedColumns, pImputeOption, pNLcolumns, pNLtechnique, pTarinTest, pSwitch, airline, sentiment):
        ...
        def imputarCoords(pml_dataset):
            ...
            def get_latitude(location, timezone, airline, airline_sentiment):
                try:
            ...
            def get_longitude(location, timezone, airline, airline_sentiment):
                try:
                    if location and airline == "Southwest" and airline_sentiment == "negative":
                        # Concatenar la ubicación y la zona horaria en un formato reconocible por el geocodificador
                        query = f"{location}, {timezone}"
        ...
        pMl_dataset = pMl_dataset.reset_index(drop=True) # Reiniciar los indices para que sean continuos
        ...
        """if pTarinTest == "test":
            ...
            if pNLtechnique == "tfidf":
                vectorizador = TfidfVectorizer()imputarCoordsech_df], axis=1)
            for columnaLN in pNLcolumns:
                pMl_dataset = pMl_dataset.drop(columnaLN, axis=1)
            with open(pNLtechnique + '.pkl', 'wb') as f:
                pickle.dump(vectorizador, f)"""
