...
class Preprocessor:
    ...
    def preprocessDataset(self, pMl_dataset, pTargetColumn, pAlgorithm, pExcludedColumns, pImputeOption, pRescaleOption):
        ...
        if pAlgorithm == "naive bayes" or pAlgorithm == "logistic regression":
            ...
            for columna in pMl_dataset:
                Preprocessor.preprocesarLenguajeNatural(columna)

                columnasLenguajeNatural = ['text']
                for columnaLN in columnasLenguajeNatural:
                    columnaTFidf = vectorizador.fit_transform(pMl_dataset[columnaLN])
                    tfidf_df = pd.DataFrame(columnaTFidf.toarray(), columns=vectorizador.get_feature_names())
                    pMl_dataset = pd.concat([pMl_dataset, tfidf_df], axis=1)
            pMl_dataset = pMl_dataset.drop('text', axis=1)
